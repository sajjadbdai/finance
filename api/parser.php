...
PROMPT;
    ...
    if ($httpCode !== 200 || !$response) {
        error_log("Parser API error: HTTP {$httpCode} response: " . substr($response,0,200));
        return [];
    }
    ...
    if ($text[0] !== '[' && $text[0] !== '{') {
        ...
        // Find last ] or }
        $end = strrpos($text, ']');
        if ($end === false) $end = strrpos($text, '}');
        if ($end !== false) $text = substr($text, 0, $end+1);
    }

    // If it's a single object, wrap in array
    if (isset($text[0]) && $text[0] === '{') $text = '[' . $text . ']';

    $parsed = json_decode($text, true);
    if (!$parsed || !is_array($parsed)) {
        error_log("Parser JSON decode failed. Raw: " . substr($rawText,0,300));
        return [];
    }
