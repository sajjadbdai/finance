...
function sendConfirmMultiple(int $chatId, array $allParsed, string $raw, string $source): void {
    ...
    foreach ($allParsed as $i => $t) {
        ...
        // Store session with unique key per transaction index
        $sessionKey  = $chatId . '_m' . $i;
        $sessionData = json_encode(['mode'=>'single','parsed'=>$t,'raw'=>$raw,'source'=>$source]);
        try {
            db()->prepare("INSERT INTO bot_sessions (telegram_id,state,context) VALUES (?,?,?) ON DUPLICATE KEY UPDATE state=VALUES(state),context=VALUES(context)")
               ->execute([$sessionKey, 'confirm', $sessionData]);
        } catch(Exception $e) {}
...
function handleCallback(array $cb): void {
    ...
    if (strpos($data, 'myes_') === 0 || strpos($data, 'mno_') === 0) {
        ...
        try {
            $st = db()->prepare("SELECT context FROM bot_sessions WHERE telegram_id=?");
            $st->execute([$sKey]);
            $row = $st->fetch();
        } catch(Exception $e) { $row = null; }

        if (!$row) { tgSend($chatId, "⏰ Session expired."); return; }

        $ctx2 = json_decode($row['context'], true);

        if ($action === 'myes') {
            $txnId = saveTransaction($ctx2['parsed'], $ctx2['source'] ?? 'telegram', $ctx2['raw'] ?? '');
            if ($txnId) {
                $acc = getAccountByName($ctx2['parsed']['account'] ?? '');
                $cur = $acc['currency'] ?? '';
...
💰 Balance: <b>{$bal} {$cur}</b>");
            ...
            tgSend($chatId, "⏭ Skipped.");
        }
        try { db()->prepare("DELETE FROM bot_sessions WHERE telegram_id=?")->execute([$sKey]); } catch(Exception $e) {}
        return;
    }
