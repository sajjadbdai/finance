...
/* Hide values - blur everything with data-hide or known value classes */
body.values-hidden [data-hide="true"],
body.values-hidden .card-value,
body.values-hidden .card-sub,
body.values-hidden .c-blue,
body.values-hidden .c-green,
body.values-hidden .c-red,
body.values-hidden .amount,
body.values-hidden .balance,
body.values-hidden .acc-bal,
body.values-hidden .cc-bal{
    filter:blur(8px) !important;
    user-select:none !important;
}
/* Hide table data cells (not headers or action columns) */
body.values-hidden .tbl td:not(:first-child):not(:last-child),
body.values-hidden table td:nth-child(2),
body.values-hidden table td:nth-child(3),
body.values-hidden table td:nth-child(4){
    filter:blur(8px) !important;
    user-select:none !important;
}
</style>
...
<nav class="sidebar" id="sidebar">
  ...
  <a href="dashboard_settings.php" class="ni <?=$activePage==='settings'?'active':'';?>"><span>⚙️</span> Settings</a>
...
document.addEventListener('DOMContentLoaded',function(){
    ...
    <div id="pin-modal" style="display:none;position:fixed;inset:0;background:#000000cc;z-index:9999;display:none;align-items:center;justify-content:center;">
      <div style="background:var(--s1);border-radius:16px;padding:28px 24px;width:280px;text-align:center;box-shadow:0 8px 32px #00000080;">
        <div id="pin-title" style="font-weight:700;font-size:1rem;margin-bottom:6px;">🔒 Enter PIN</div>
        <div id="pin-subtitle" style="font-size:.8rem;color:var(--muted);margin-bottom:20px;">Enter 4-digit PIN to view values</div>
        <div id="pin-dots" style="display:flex;justify-content:center;gap:12px;margin-bottom:24px;">
          <div class="pin-dot" style="width:14px;height:14px;border-radius:50%;border:2px solid var(--muted);background:transparent;transition:.2s;"></div>
          <div class="pin-dot" style="width:14px;height:14px;border-radius:50%;border:2px solid var(--muted);background:transparent;transition:.2s;"></div>
          <div class="pin-dot" style="width:14px;height:14px;border-radius:50%;border:2px solid var(--muted);background:transparent;transition:.2s;"></div>
          <div class="pin-dot" style="width:14px;height:14px;border-radius:50%;border:2px solid var(--muted);background:transparent;transition:.2s;"></div>
        </div>
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:12px;">
          ...
          <button onclick="pinKey('${k}')" style="padding:14px;font-size:1.1rem;font-weight:600;border-radius:10px;border:1px solid var(--s3);background:var(--s2);color:var(--text);cursor:pointer;${k===''?'visibility:hidden':''}">
            ${k}
          </button>`).join('')}
        </div>
        <button onclick="closePinModal()" style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--s3);background:none;color:var(--muted);cursor:pointer;font-size:.85rem;">Cancel</button>
      </div>
    </div>
    <div id="pin-setup-modal" style="display:none;position:fixed;inset:0;background:#000000cc;z-index:9999;align-items:center;justify-content:center;">
      <div style="background:var(--s1);border-radius:16px;padding:28px 24px;width:280px;text-align:center;">
        <div style="font-weight:700;font-size:1rem;margin-bottom:6px;">🔐 Setup PIN</div>
        <div id="setup-subtitle" style="font-size:.8rem;color:var(--muted);margin-bottom:20px;">Enter new 4-digit PIN</div>
        <div id="setup-dots" style="display:flex;justify-content:center;gap:12px;margin-bottom:24px;">
          <div class="setup-dot" style="width:14px;height:14px;border-radius:50%;border:2px solid var(--muted);background:transparent;transition:.2s;"></div>
          <div class="setup-dot" style="width:14px;height:14px;border-radius:50%;border:2px solid var(--muted);background:transparent;transition:.2s;"></div>
          <div class="setup-dot" style="width:14px;height:14px;border-radius:50%;border:2px solid var(--muted);background:transparent;transition:.2s;"></div>
          <div class="setup-dot" style="width:14px;height:14px;border-radius:50%;border:2px solid var(--muted);background:transparent;transition:.2s;"></div>
        </div>
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:12px;">
          ${[1,2,3,4,5,6,7,8,9,'',0,'⌫'].map(k=>`
          <button onclick="setupPinKey('${k}')" style="padding:14px;font-size:1.1rem;font-weight:600;border-radius:10px;border:1px solid var(--s3);background:var(--s2);color:var(--text);cursor:pointer;${k===''?'visibility:hidden':''}">
            ${k}
          </button>`).join('')}
        </div>
        <button onclick="closeSetupPin()" style="width:100%;padding:10px;border-radius:8px;border:1px solid var(--s3);background:none;color:var(--muted);cursor:pointer;font-size:.85rem;">Cancel</button>
      </div>
    </div>
    `);

    // ── Hide values by default on page load (if enabled) ───
    var hideEnabled = localStorage.getItem('hide_on_load') !== 'false'; // default ON
    if(hideEnabled){
        document.body.classList.add('values-hidden');
        document.getElementById('hide-btn').textContent='🙈';
    } else {
        _hidden = false;
        document.getElementById('hide-btn').style.display = 'none'; // hide the button too
    }

    // Long press hide button to setup PIN
    var pressTimer;
    var btn = document.getElementById('hide-btn');
    if(btn){
        btn.addEventListener('touchstart', function(){ pressTimer=setTimeout(openSetupPin,800); });
        btn.addEventListener('touchend',   function(){ clearTimeout(pressTimer); });
        btn.addEventListener('mousedown',  function(){ pressTimer=setTimeout(openSetupPin,800); });
...
function toggleHide(){
    var hideEnabled = localStorage.getItem('hide_on_load') !== 'false';
    if(!hideEnabled) return; // feature disabled
    if(!_hidden){
        // Hide - no PIN needed
        _hidden = true;
        document.body.classList.add('values-hidden');
        ...
        // Show - check PIN
        var pin = localStorage.getItem('finance_pin') || '';
        if(!pin){
            // No PIN set - show directly
            showValues();
        } else {
            openPinModal();
        }
...
function openPinModal(){
    _pinEntry = '';
    updateDots('pin-dots','pin-dot');
    document.getElementById('pin-modal').style.display='flex';
    document.getElementById('pin-subtitle').textContent='Enter 4-digit PIN to view values';
    document.getElementById('pin-subtitle').style.color='var(--muted)';
}
...
function pinKey(k){
    if(k==='⌫'){ _pinEntry=_pinEntry.slice(0,-1); }
    else if(k!=='' && _pinEntry.length<4){ _pinEntry+=k; }
    updateDots('pin-dots','pin-dot');
    if(_pinEntry.length===4){
        ...
        if(_pinEntry===pin){
            ...
            document.getElementById('pin-subtitle').textContent='Wrong PIN! Try again.';
            document.getElementById('pin-subtitle').style.color='var(--red)';
            _pinEntry='';
            setTimeout(function(){ updateDots('pin-dots','pin-dot'); },300);
        }
    }
}

// ── PIN Setup ─────────────────────────────────────────────
var _setupEntry=''; var _setupFirst=''; var _setupStep=1;

function openSetupPin(){
    _setupEntry=''; _setupFirst=''; _setupStep=1;
    document.getElementById('setup-subtitle').textContent='Enter new 4-digit PIN';
    updateDots('setup-dots','setup-dot');
...
function setupPinKey(k){
    if(k==='⌫'){ _setupEntry=_setupEntry.slice(0,-1); }
    else if(k!=='' && _setupEntry.length<4){ _setupEntry+=k; }
    updateDots('setup-dots','setup-dot');
    if(_setupEntry.length===4){
        if(_setupStep===1){
            _setupFirst=_setupEntry; _setupEntry=''; _setupStep=2;
            document.getElementById('setup-subtitle').textContent='Confirm PIN';
            setTimeout(function(){ updateDots('setup-dots','setup-dot'); },300);
        } else {
            if(_setupEntry===_setupFirst){
                localStorage.setItem('finance_pin',_setupFirst);
                closeSetupPin();
                // Show checkmark briefly
                var btn=document.getElementById('hide-btn');
                btn.textContent='✅'; setTimeout(function(){ btn.textContent='🙈'; },1500);
            } else {
                document.getElementById('setup-subtitle').textContent="PINs don't match! Try again.";
                document.getElementById('setup-subtitle').style.color='var(--red)';
                _setupEntry=''; _setupFirst=''; _setupStep=1;
                setTimeout(function(){ updateDots('setup-dots','setup-dot'); },300);
            }
        }
...
function updateDots(containerId, dotClass){
    var dots=document.querySelectorAll('.'+dotClass);
    var entry=containerId==='pin-dots'?_pinEntry:_setupEntry;
    dots.forEach(function(d,i){
        d.style.background = i<entry.length ? 'var(--blue)' : 'transparent';
        d.style.borderColor= i<entry.length ? 'var(--blue)' : 'var(--muted)';
    });
}
